...
export function LoginPage() {
  ...
  const { login, user, intendedPath, setIntendedPath } = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // On success, we keep loading state true and let the useEffect handle navigation
  ...
  // Auto-redirect if already logged in / login success
  useEffect(() => {
    if (user) {
      if (user.role === 'driver') {
        navigate('/driver');
  ...
  }, [user, navigate, intendedPath, setIntendedPath]);
